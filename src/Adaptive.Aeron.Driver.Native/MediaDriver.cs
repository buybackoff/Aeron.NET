...
namespace Adaptive.Aeron.Driver.Native
{
    public class MediaDriver : CriticalFinalizerObject, IDisposable
    {
        ...
        private MediaDriver(MediaDriverConfig config)
        {
            ...
            _clientCountersFile =
                new FixedSizeDirectFile(Path.Combine(dirPath, MediaDriverConfig.ClientCountersFileName));
        }
        ...
        public int GetNewClientStreamId()
        {
            var id = Interlocked.Increment(
                ref Unsafe.As<byte, int>(ref _clientCountersFile.Span[MediaDriverConfig.ClientStreamIdCounterOffset]));
